...
io.on("connection", (socket) => {
  socket.on("create", ({ room, maxPlayers }) => {
    ...
    // Lobby neu anlegen, wenn sie nicht existiert ODER leer ist
    if (!games[room]) newRoom(room, maxPlayers);
    else if (Object.keys(games[room].players || {}).length === 0) newRoom(room, maxPlayers);
  ...
  socket.on("endTurn", ({ room }) => {
    ...
    // PASS erlaubt, wenn kein legaler Zug
